  ...
  function detect(page) {
    ...
    if (context.length && strong.length && termsLinks.length) {
      return {
        ...
        score: consentScore, context, termsLinks, density, length,
      };
    }
    return { kind: 'none', reason: '', score: Math.max(docScore, consentScore), context, termsLinks, density, length };
  }
